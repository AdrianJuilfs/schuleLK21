...
public class Tierdatenbank1 extends JFrame {
	...
	private void datenbankAbfrage() {
		try (
			...
			ResultSet rs = stmt.executeQuery("SELECT name, tierart, vorname "
					+ "FROM tier, beziehung, besitzer where tier_id = beziehung_tier_id and besitzer_id = beziehung_besitzer_id ");
			while (rs.next()) {
				tierliste.addElement(rs.getString("name") + ", " + rs.getString("tierart") + ", " + rs.getString("vorname"));
			}
	...
	public static void main(final String[] args) {
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					new Tierdatenbank1("Haustier-Datenbank");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
