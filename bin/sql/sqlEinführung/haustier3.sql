eDROP SCHEMA IF EXISTS haustier;
...
SELECT vorname,nachname,name FROM tier, besitzer
WHERE besitzer_id = tier_id
ORDER BY name ASC;


# 2B) Liste alle Besitzer von Maja auf (Vor- und Nachname).
SELECT vorname,nachname FROM tier,besitzer
WHERE besitzer_id = tier_id
AND name = 'Maja';

# 2C) Zähle die Anzahl der Besitzer von Blacky.
SELECT COUNT(*) FROM tier,besitzer
WHERE besitzer_id = tier_id
GROUP BY name
HAVING name = 'Blacky';
...
SELECT DISTINCT vorname,nachname, COUNT(*)
FROM tier,besitzer
WHERE besitzer_id = tier_id
GROUP BY name;
...
SELECT * FROM besitzer WHERE telefonnr IS NULL;

# 2F) Liste Straße, PLZ und Ort aller Besitzer auf. Dabei soll jede Adresse nur einmal
#     ausgegeben werden.
SELECT DISTINCT straße,plz,ort FROM besitzer;
